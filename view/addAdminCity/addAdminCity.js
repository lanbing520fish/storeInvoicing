angular
    ...
    .run(['$rootScope', function($rootScope) {
        $rootScope.STAFF_ID = ''; // 后台调用session获取登录管理员ID
    }])
    .run(['$rootScope', function($rootScope) {
    ...
    .controller('cityCheckCtrl', ['$scope', '$rootScope', 'httpMethod', function($scope, $rootScope, httpMethod) {
        httpMethod.qryProvinceRegion({
            'staffId': $rootScope.STAFF_ID
        }).then(function(rsp) {
            if (rsp.success) {
                httpMethod.qryRegion({
                    'commonRegionId': rsp.data.commonRegionId
                }).then(function(rsp) {
                    if (rsp.success) {
